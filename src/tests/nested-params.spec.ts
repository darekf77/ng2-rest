...
describe('Nested params', () => {
  ...
  it('should retrive proper names of rest query params', () => {
    ...
    expect(UrlNestedParams.getRestPramsNames(pattern)).to.deep.eq([
      'bookId',
      'author',
    ]);
  });
  ...
  it('should retrive pairs value/key for rest query params 3 level', () => {
    let pattern =
      'http://something.com/book/:bookId/author/:author/withCover/:withCoverVal';
  ...
  it('should retrive pairs value/key for rest query params 3 level other case', () => {
    let title = 'aaasdasd';
    let pattern =
      'http://something.com/books/:bookid/title/:titleId/mission/:missionId';
